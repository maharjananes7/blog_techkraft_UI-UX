...
const latestbloglist = document.getElementById('latest_blog');
const blogPosts = [{
    "author": "Dan Peck",
...
blogPosts.forEach(item => {
    const single_blog = `
    <li>
    <img class="bloglist-img" src="${item.image}" alt="image">
    <a class="blog-list-title" onclick="to_blog_detail(event, ${item.title})">${item.title}</a>
    <span class="blogList-author">
        <h3>${item.author}</h3>
        <h3>${item.published_at.substring(0, 10)}</h3>
    </span>
</li>`
    blogList.innerHTML += single_blog;
})
function to_blog_detail(event, data) {
    event.preventDefault();
    window.open(`blog-detail.html?title=${data}`, '_blank');
}
